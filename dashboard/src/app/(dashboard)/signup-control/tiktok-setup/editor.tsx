...
export function TiktokSetupEditor({
  ...
  return (
    <EditorShell
      ...
      value={c}
      onChange={setC}
      onSave={() => savePageContent("tiktok-setup", c)}
    >
      {updatedAt && (
            ...
            <div className="space-y-3">
              <FieldRow>
                <SelectField
                  label="Step icon"
                  value={step.icon}
                  onChange={(v) => set({ ...step, icon: v })}
                  options={STEP_ICONS}
                />
                <TextField
                  label="Badge (optional)"
                  hint="Small badge under the title — emoji + short text. Leave blank to hide."
                  value={step.badge}
                  onChange={(v) => set({ ...step, badge: v })}
                  {...styleProps(`steps.${i}.badge`)}
                />
              </FieldRow>
              ...
              <TextAreaField
                ...
                onChange={(v) => set({ ...step, description: v })}
                {...styleProps(`steps.${i}.description`)}
              />
              ...
              {/* Platform-aware sections (sub-cards with their own videos/downloads) */}
              <div className="space-y-1">
                    ...
                    <div className="space-y-3">
                      <FieldRow>
                        <SelectField
                          label="Platform"
                          value={s.platform}
                          onChange={(v) => setS({ ...s, platform: v })}
                          options={SECTION_PLATFORMS}
                        />
              ...
              <div className="space-y-1">
                ...
                <StringList
                  ...
                  onChange={(v) => set({ ...step, instructions: v })}
      ...
      <Section title="Final-step footer (Complete Application + Need Help)">
        <TextField
          label="Heading"
          value={c.finalFooter.heading}
          onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, heading: v })}
          {...styleProps("finalFooter.heading")}
        />
        ...
        <div className="space-y-2 rounded-lg border border-emerald-500/20 bg-white/50 p-3">
          ...
          <TextField
            label="Heading"
            value={c.finalFooter.completeApp.heading}
            onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, completeApp: { ...c.finalFooter.completeApp, heading: v } })}
            {...styleProps("finalFooter.completeApp.heading")}
          />
          <TextAreaField
            label="Body"
            rows={3}
            value={c.finalFooter.completeApp.body}
            onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, completeApp: { ...c.finalFooter.completeApp, body: v } })}
            {...styleProps("finalFooter.completeApp.body")}
          />
          <FieldRow>
            <TextField
              label="Button label"
              value={c.finalFooter.completeApp.signUpLabel}
              onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, completeApp: { ...c.finalFooter.completeApp, signUpLabel: v } })}
              {...styleProps("finalFooter.completeApp.signUpLabel")}
            />
            <UrlField
              label="Button link"
              value={c.finalFooter.completeApp.signUpHref}
              onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, completeApp: { ...c.finalFooter.completeApp, signUpHref: v } })}
            />
          </FieldRow>
        </div>
        ...
        <div className="space-y-2 rounded-lg border border-emerald-500/20 bg-white/50 p-3">
          ...
          <TextField
            label="Heading"
            value={c.finalFooter.needHelp.heading}
            onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, needHelp: { ...c.finalFooter.needHelp, heading: v } })}
            {...styleProps("finalFooter.needHelp.heading")}
          />
          <TextAreaField
            label="Body"
            rows={2}
            value={c.finalFooter.needHelp.body}
            onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, needHelp: { ...c.finalFooter.needHelp, body: v } })}
            {...styleProps("finalFooter.needHelp.body")}
          />
          <FieldRow>
            <TextField
              label="Email address"
              value={c.finalFooter.needHelp.email}
              onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, needHelp: { ...c.finalFooter.needHelp, email: v } })}
              {...styleProps("finalFooter.needHelp.email")}
            />
            <TextField
              label="Email button label"
              value={c.finalFooter.needHelp.emailLabel}
              onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, needHelp: { ...c.finalFooter.needHelp, emailLabel: v } })}
              {...styleProps("finalFooter.needHelp.emailLabel")}
            />
          </FieldRow>
          <FieldRow>
            <UrlField
              label="WhatsApp URL"
              value={c.finalFooter.needHelp.whatsappUrl}
              onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, needHelp: { ...c.finalFooter.needHelp, whatsappUrl: v } })}
            />
            <TextField
              label="WhatsApp button label"
              value={c.finalFooter.needHelp.whatsappLabel}
              onChange={(v) => setSection("finalFooter")({ ...c.finalFooter, needHelp: { ...c.finalFooter.needHelp, whatsappLabel: v } })}
              {...styleProps("finalFooter.needHelp.whatsappLabel")}
            />
          </FieldRow>
        </div>
      </Section>
      ...
      <HistoryPanel<TiktokSetupContent> slug="tiktok-setup" onRestore={setC} />
    </EditorShell>
  );
}
