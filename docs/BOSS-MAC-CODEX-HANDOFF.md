...
Only after I explicitly approve activation, set CREDDY_PIPELINE_ENABLED=true and create exactly one hourly Codex scheduled task in the Social Media Automation project. Its workflow must follow `docs/CREDDY-SCHEDULED-TASKS.md`: collect and rank only new/changed inputs hourly, persist one daily slate of zero to five stories on the first successful run after 06:00 America/New_York, preserve every JSON/Markdown report, and keep normal social delivery in human review. The separately gated, fully verified `auto_urgent` lane is the only unattended social exception. Never create one scheduled task per agent and never schedule the standalone News repair CLI.

The scheduled task must begin with `npm run creddy:validate` and `npm run creddy:test`, then use the exact hourly orchestration and Agent 01→08 sequence from `docs/CREDDY-SCHEDULED-TASKS.md`. It must finish with `npm run creddy:pipeline -- report` and a visible summary of every stage, retry, no-op, blocker, output path, Content Bank status, Slack status, and exact next human action. Missing normal social approval is a safe no-op, not an error.
...
Keep the Mac powered on and the ChatGPT/Codex desktop app running for local scheduled tasks. After creating the scheduler, show its name, project, local execution mode, exact local time zone and hourly cadence, next run, ACTIVE/PAUSED state, and confirmation that there is only one matching Creddy automation. Do not create a duplicate if one already exists; update the existing one.
```
